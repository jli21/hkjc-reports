...
\item \textbf{Change the shrinkage target from the population to a group,
everywhere a per-horse history is summarised.} This is the one structural change
the study endorses without qualification, because it costs nothing: the same
per-horse mean, pulled toward its group instead of toward zero, improves forward
correlation by 12\% relative (0.255 to 0.285). It is a one-line change to any
shrinkage estimator. Its production value did not survive canonical confirmation as
a \emph{feature} ($-0.00085$, $t=-0.90$), but as a \emph{method} it should be the
default for the reference project's existing state engines, whose
\texttt{*\_reliability} fields and shrunk means all pull toward a global mean today.

\item \textbf{Use group-level volatility rather than per-horse volatility.} The
style group's volatility forecasts a horse's next deviation better than the horse's
own history does ($+0.112$ against $+0.101$), on 70{,}507 rows. This is the first
volatility construction in the project that beats the individual, and it is the
right form for any future use --- including in staking, where a variance estimate
matters more than in the probability.

\item \textbf{Take the sectional finishing band to canonical confirmation.}
$-0.00087$ on the development profile is 20\% of the production edge with the right
sign in three of five years, and it is now the best-measured candidate whose
canonical run has not been spent. Given what canonical confirmation did to the
prior-class block, the prior should be that roughly half the effect survives.
\end{enumerate}

\subsection*{Moderate evidence --- worth one more experiment}

\begin{enumerate}\setcounter{enumi}{3}\itemsep3pt
\item \textbf{The horse's historical class level, in some form.} It produced the
only development-gate pass in the study ($t=-3.80$, 5/5 years) and lost half its
effect at canonical scale. The production set has today's class and the step
between runs, but not the level a horse has been campaigning at, and that gap is
real even if this particular encoding of it is not yet promotable.
